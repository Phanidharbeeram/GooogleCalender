...
app.post("/events", (req, res) => {
  ...
  calender.freebusy.query(
    ...
    (err, res) => {
      if (err) return console.log("Free Busy err" + err);
      const eventArr = res.data.calender.primary.busy;
      if (eventArr.length === 0) {
        return calender.events.insert(
          ...
          (err) => {
            if (err) return console.log("Errror creating calender event" + err);
            return console.log("event created !!");
          }
        );
      }
      return console.log("sorry event already exists for the day");
    }
  );
  ...
  const msg = {
    to: req.body.to,
    from: "phanidharbeeram@gmail.com",
